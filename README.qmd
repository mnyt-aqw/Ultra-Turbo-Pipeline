...
If you need to change the amout of resources allocated to each process just open the `nextflow.config` file and change the values.
```groovy
process {
    withName:PRODIGAL {
        cpus = 1 // nr CPUs
...
- `input_files`: Path to input Pod5 files, or basecalled fasta files, depending on which modules you want ot run.
...
- `gpu_allocated`: If you want to run Dorado using a GPU you can specify which one here. IF you run the pipeline on the Vera cluster you write `--gpus-per-node=A40:1`. This will allocate one A40 GPU per process. 
