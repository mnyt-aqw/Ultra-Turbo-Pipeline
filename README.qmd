...
* Converts fast5 files to pod5 format and splits reads by channel for parallel processing.
* Basecalls reads from each channel individually using Dorado.
* Combines base-called reads and performs quality control using PycoQC and Filtlong.
* Assembles genomes with Flye and polishes assemblies with Medaka.
* Assesses assembly quality with CheckM2 and BUSCO.
* Screens for antimicrobial resistance genes (ARGs) and mobile genetic elements (MGEs).
...
The pipeline can be configured using the following parameters in nextflow.config:
...
* `params.databases`: Path to the databases for Diamond in protein format.
* `params.clusterOptions`: Cluster options for SLURM or other schedulers.
...
* `params.diamond_subject_cov`: Minimum subject coverage threshold for Diamond.
...
* `params.filtlong_keep_percent`: Percentage of reads to keep for Filtlong.
* `params.DORADO_device`: Device to use for Dorado (e.g., CPU or GPU).
* `params.DORADO_kit`: Kit used for Dorado basecalling.

## Process Configuration

The pipeline includes specific configurations for each process, such as CPU requirements, Conda environments, container images, and error handling strategies. For example:
